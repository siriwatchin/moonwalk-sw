...
Produces the exact same payload format the firmware sends, paced at 50 ms, so the
dashboard can be developed and demoed with no Nano present. Implements SensorSource.
"""
...
from .config import GRAVITY, PAYLOAD_TAG
...
class MockNanoSource:
    """Yields mock IMU CSV lines, cycling through cane states with noise."""

    def __init__(self, cycle: Optional[list[StateProfile]] = None,
                 seed: Optional[int] = None, interval_ms: int = 50, paced: bool = True):
